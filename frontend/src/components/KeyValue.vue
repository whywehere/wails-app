<template>
  <main v-if="dbStore.keyKey !== undefined">
    <el-card class="box-card">
      ...
      <span class = "text item">过期时间: {{form.ttl}}</span>
      <span class = "text item">数据类型: {{form.type}}</span>
    </el-card>
    <el-form :model="form">
<!--      <el-form-item label="键">-->
<!--&lt;!&ndash;        <span v-model="form.key"> {{form.key}}</span>&ndash;&gt;-->
<!--        <el-input autosize v-model="form.key" disabled placeholder="" />-->
<!--      </el-form-item>-->
<!--      <el-row>-->
<!--        <el-col :span="11">-->
...
<!--      </el-row>-->
      <div v-if="form.type === 'string'">
        <el-form-item label="值">
          <el-input type="textarea" :autosize="{ minRows: 2 }" v-model="form.value" placeholder="" />
        </el-form-item>
        <el-form-item>
...
watch(()=>dbStore.keyKey, ()=>{
    getTheValue()
  })

function getTheValue() {
  GetKeyValue({conn_identity:dbStore.keyConnIdentity, db: dbStore.keyDB, key: dbStore.keyKey}).then(res => {
    if (res.code !== 200) {
      ElNotification({
        title:res.msg,
        type: "error",
    ...
    form.value = res.data
    form.value.key = dbStore.keyKey
    console.log("form.value.key: ", form.value.key)
  })
}

function updateKey() {
  UpdateKeyValue({conn_identity: dbStore.keyConnIdentity, key: dbStore.keyKey, db: dbStore.keyDB, value: form.value.value, ttl: form.value.ttl}).then(res => {
    if (res.code !== 200) {
      ElNotification({
        title:res.msg,
        type: "error",
...
function deleteHashField(fields) {
  HashFieldDelete({conn_identity: dbStore.keyConnIdentity, db: dbStore.keyDB, key: dbStore.keyKey, field:fields}).then(res => {
    if (res.code !== 200) {
      ElNotification({
        title:res.msg,
        type: "error",
...
function hashFiledChange () {
  HashAddOrUpdateField({conn_identity: dbStore.keyConnIdentity, db: dbStore.keyDB, key: dbStore.keyKey, field: hashForm.value.field, value: hashForm.value.value}).then(res => {
...
function deleteListItem(value) {
  ListValueDelete({conn_identity: dbStore.keyConnIdentity, db: dbStore.keyDB, key: dbStore.keyKey, value: value}).then(res => {
    if (res.code !== 200) {
      ElNotification({
        title:res.msg,
        type: "error",
...
function createListValue() {
  ListValueCreate({conn_identity: dbStore.keyConnIdentity, db: dbStore.keyDB, key: dbStore.keyKey, value: listForm.value.value}).then(res => {
...
function deleteSetItem(value) {
  SetValueDelete({conn_identity: dbStore.keyConnIdentity, db: dbStore.keyDB, key: dbStore.keyKey, value: value}).then(res => {
    if (res.code !== 200) {
      ElNotification({
        title:res.msg,
        type: "error",
...
function createSetValue() {
  SetValueCreate({conn_identity: dbStore.keyConnIdentity, db: dbStore.keyDB, key: dbStore.keyKey, value: setForm.value.value}).then(res => {
...
function deleteZSetMember(member) {
  ZSetValueDelete({conn_identity: dbStore.keyConnIdentity, db: dbStore.keyDB, key: dbStore.keyKey, member: member}).then(res => {
    if (res.code !== 200) {
      ElNotification({
        title:res.msg,
        type: "error",
...
function createZSetValue() {
  ZSetValueCreate({conn_identity: dbStore.keyConnIdentity, db: dbStore.keyDB, key: dbStore.keyKey, score: +(zsetForm.value.score), member: zsetForm.value.member}).then(res => {
